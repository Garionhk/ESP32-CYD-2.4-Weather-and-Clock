...
// --- Configuration ---
const char* ssid = "YourWIFI2.4G";
const char* password = "YourWIFIPWDg";
...
// Type in your City's Coordinates
...
const long screenInterval = 10000; 
...
void displayJustTime() {
  ...
  // Date (Size 1) at the bottom, reduced to 80% width (approx 256px)
  tft.setTextSize(1);
  int dateWidth = 256;
  int startX = (320 - dateWidth) / 2;
  float charSpacing = (float)dateWidth / 10.0;
  
  // Clear the date area first to allow transparent drawing
  tft.fillRect(0, 155, 320, 85, TFT_BLACK); 
  tft.setTextColor(TFT_GREEN); 
  
  for (int i = 0; i < 10; i++) {
    char buf[2] = {dateStr[i], 0};
    // Add 2px additional spacing for each digit after the first
    int offsetX = (i > 0) ? (i * 2) : 0;
    tft.drawString(buf, (int)(startX + (i * charSpacing)) + offsetX, 160, 7);
  }
}
